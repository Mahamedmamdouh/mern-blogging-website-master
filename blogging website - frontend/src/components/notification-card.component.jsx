...
const NotificationCard = ({ data, index, notificationState }) => {
      ...
      let {seen,type,reply,comment,createdAt,replied_on_comment,user,user:{personal_info:
            {fullname,username,profile_img}},blog:{_id,blog_id,title},_id:notification_id} = data;
            
      let {userAuth:{username:author_username,profile_img:author_profile_img,
      access_token }} = useContext(UserContext);
            ...
            <div className={`p-6 border-b border-grey border-l-black ${!seen && "border-l-2"}`}>
                  <div className="flex gap-5 mb-3">
                        ...
                        <div className="w-full">
...

                              {
                                    ...
                                    :
                                    <Link to={`/blog/${blog_id}`} className="font-medium text-dark-grey
                                     hover:underline line-clamp-1" >{`"${title}"`}</Link>
                              }
                  ...
                  <div className="ml-4 pl-5 mt-3 text-dark-grey flex gap-8">
                        ...
                        <p>{getDay(createdAt)}</p>
                        {
                              ...
                              <button className="underline hover:text-black"
                              onClick={(e=>handleDelete(comment._id,"comment",e.target))}> Delete</button>
                              </> : ''
...

                  {
                        isReplaying ?
                        <div className="mt-8">
                              <NotificationCommentField _id={_id} blog_author={user} index={index}
                               replyingTo={comment._id} setReplying={setIsReplaying} notification_id={notification_id}
...

                  {
                        ...
                        <div className="ml-20 p-5 bg-grey mt-5 rounded-md">
                              <div className="flex gap-3 mb-3">
                                    <img src={author_profile_img} className="w-8 h-8 
                                    rounded-full"/>
                                    <div>
                                          <h1 className="font-medium text-xl 
                                          text-dark-grey">
                                                <Link to={`/user/${author_username}`}> @{author_username}
                                                </Link>
                              ...
                              onClick={(e=>handleDelete(comment._id,"reply",e.target))}> Delete</button>
